...
var posts,
    ...
    config = {
        ...
        salt: '',
        content: process.cwd(),
        outputPath: Path.join(process.cwd(), '_dnote'),
        pdf: true,
        generated: false,
...
renderer.link = function(href, title, text) {
    if (href.indexOf('http') == -1) {
        var lid = '',
            baseLink = href;
        var fileCategory = internals.sanitizeCategory(href);
        if (href.indexOf('#') != -1) {
...
internals.processContent = function(config, cb) {
    ...
    glob("**/*.md", {
        ...
        files = _.filter(files, function(file) {
            return Path.basename(file) != "README.md" && file.indexOf('_dnote') == -1
        });
        var p = [];
...
exports.start = function(c) {
    ...
    internals.processContent(config, internals.start);
};
/* 
...
internals.rmdir = function(path) {
    if (fs.existsSync(path)) {
        fs.readdirSync(path).forEach(function(file, index) {
            ...
            if (fs.lstatSync(cp).isDirectory()) { // recurse
                internals.rmdir(cp);
            } else { // delete file
                fs.unlinkSync(cp);
            }
...
internals.generate = function(c) {
    if (fs.existsSync(c.outputPath)) {
        internals.rmdir(c.outputPath);
    }
    fs.mkdir(c.outputPath, function(err) {
        if (err) throw err;
        ...
        for (var i = posts.categories.length - 1; i >= 0; i--) {
            ...
            for (var j = ps.length - 1; j >= 0; j--) {
                ...
                fs.writeFile(Path.join(c.outputPath, category + '-' + context.posts[context.pindex].metadata.url + '.html'), html, function(err) {
                    if (err) throw err;
                });
            };
        };
        fs.mkdirSync(Path.join(c.outputPath,'static'));
        fs.mkdirSync(Path.join(c.outputPath,'static','files'));
        ncp(Path.join(__dirname,'../static/'),Path.join(c.outputPath,'static','files'));
    });
};

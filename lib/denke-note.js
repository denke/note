...
internals.generate = function(c) {
    if (fs.existsSync(Path.join(config.outputFolder, config.outputPath))) {
        internals.rmdir(Path.join(config.outputFolder, config.outputPath));
    }
    fs.mkdir(Path.join(config.outputFolder, config.outputPath), function(err) {
        if (err) throw err;
        ...
        for (var i = posts.categories.length - 1; i >= 0; i--) {
            ...
            for (var j = ps.length - 1; j >= 0; j--) {
                ...
                fs.writeFile(Path.join(config.outputFolder, config.outputPath, category + '-' + context.posts[context.pindex].metadata.token + '.html'), html, function(err) {
                    if (err) throw err;
                });
                if (j==0) {
                   fs.writeFile(Path.join(config.outputFolder, config.outputPath, category + '.html'), html, function(err) {
                        if (err) throw err;
                    }); 
                };
            };
        };
        fs.mkdirSync(Path.join(config.outputFolder, config.outputPath, 'static'));
        fs.mkdirSync(Path.join(config.outputFolder, config.outputPath, 'static', 'files'));
        ncp(Path.join(__dirname, '../static/'), Path.join(config.outputFolder, config.outputPath, 'static', 'files'));
    });
};
...
internals.init = function (c) {
    ncp(Path.join(__dirname, '../docs/sample/'), Path.join(config.outputFolder));
    console.log('Some sample documents have been created at: ',Path.join(config.outputFolder));

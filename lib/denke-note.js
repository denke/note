...
/*
 * Custom `marked` renderer to parse code and mermaid diagrams.
 *
 * @param {String} href
 * @param {String} title
 * @param {String} text
...
renderer.code = function(code, language) {
    if (language && language.match(/^diagram/)) {
        return '<div class="mermaid" style="text-align: center">' + code + '</div>';
    } else {
...
Handlebars.registerHelper('category_name', function() {
    try {
        ...
        var name = Handlebars.escapeExpression(this);
        return new Handlebars.SafeString(name.charAt(0).toUpperCase() + name.slice(1));
    }
});
/*
 * handlebar helper to minify code
 */
Handlebars.registerHelper('minify', function(options) {
    options = _.extend(options, options.hash || {}, {
    ...
    return new Handlebars.SafeString(minifier(options.fn(this), options));
});
/*
 * handlebar helper to display category names.
 */
...
/*
 * HAPI Request handler to display an article Denke Note webpage.
 * It adds the CSS inline and does not uses JavaScript, so the HTML is self contained.
 *
 * @param {Object} request
 * @param {Object} reply
 */
internals.getArticle = function(request, reply) {
    var category = request.params.url.split(':')[0],
        url = request.params.url.split(':')[1];
    var context = internals.findPost(category, url);
    if (context == {}) {
        return reply.view('404', context);
    } else {
        context.inlineStyle = fs.readFileSync(Path.join(__dirname, '../bower_components/bootswatch-dist/css/bootstrap.min.css'), 'utf8') + fs.readFileSync(Path.join(__dirname, '../bower_components/ionicons/css/ionicons.min.css'), 'utf8') + fs.readFileSync(Path.join(__dirname, '../static/main.css'), 'utf8') + fs.readFileSync(Path.join(__dirname, '../static/mermaid.css'), 'utf8');
        return reply.view('article', context);
    }
};
/*
...
internals.printPDF = function(request, reply) {
    ...
    try {
        ...
        var js = fs.readFileSync(Path.join(__dirname, '../static/mermaid.min.js'), 'utf8')
        var css = fs.readFileSync(Path.join(__dirname, '../bower_components/bootswatch-dist/css/bootstrap.min.css'), 'utf8');
        css += fs.readFileSync(Path.join(__dirname, '../static/mermaid.pdf.css'), 'utf8');
        css += fs.readFileSync(Path.join(__dirname, '../static/pdf.css'), 'utf8');
        var template = Handlebars.compile(templatePdf);
        var text = context.posts[context.pindex].text;
        if (request.params.url === '*') {
            text = '';
            _.each(context.posts, function(p) {
                text += p.text + '\r\n';
            });
        }
...
internals.start = function(config) {
    ...
    portfinder.getPort(function(err, port) {
        ...
        server.route({
            method: 'GET',
            path: listpath + 'article/{url}',
            handler: internals.getArticle
        });
        server.route({
            method: 'GET',
            path: listpath,

...
renderer.heading = function(text, level) {
    ...
    var id = shorthash.unique(config.salt + moment())+'-'+ escapedText;
    return '<h' + level + ' id="' + id + '">' + text + '<a href="#' + id + '" class="hplk"><i class="ion-link"></i></a></h' + level + '>';
};
...
Handlebars.registerHelper('date_from_now', function(d) {
    try {
        ...
        return new Handlebars.SafeString(
            moment(date, config.dateFormat).fromNow()
        );
    } catch (e) {
...
internals.processContent = function(content) {
    ...
    glob("**/*.md", {
        ...
        _.forEach(files, function(file) {
            ...
            p[fileCategory].push(internals.getMarkdownFile(Path.join(content, file)));
        });
